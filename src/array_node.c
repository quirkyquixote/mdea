...
int mdea_array_node_serialize(void *p, struct mdea_emitter *e, int indent, char **error)
{
	struct mdea_array *array;
...
int mdea_array_node_get_number(void *p, double *number, char **error)
{
	mdea_error(error, "Expected number, got array");
	return -1;
}

int mdea_array_node_get_string(void *p, const char **string, char **error)
{
	mdea_error(error, "Expected string, got array");
	return -1;
}

int mdea_array_node_get_boolean(void *p, int *boolean, char **error)
{
	mdea_error(error, "Expected boolean, got array");
	return -1;
}
...
int mdea_array_node_get_object(void *p, struct mdea_object **object, char **error)
{
	mdea_error(error, "Expected object, got array");
	return -1;
}
